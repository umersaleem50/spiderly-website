...
This means two users editing the same record can't silently overwrite each other — the second save fails loudly instead of clobbering the first. No per-entity configuration is required. (The check runs on update; deletes use a bulk `ExecuteDeleteAsync` and are not version-guarded.)
