...
- The Angular form renders `Status` as a scalar dropdown populated from `OrderStatusCodes` — no lookup table or M2O navigation property is generated.
- `IsManyToOneType` short-circuits for properties whose type is in the `[SpiderlyEnum]` registry, so the generators that previously emitted FK plumbing skip it for these properties.
