...
The schema cannot enforce "every principal has a dependent" (that direction is always 0..1), so `[Required]` on the principal-side navigation is a build error ([SPIDERLY021](/docs/build-diagnostics#spiderly021)).
