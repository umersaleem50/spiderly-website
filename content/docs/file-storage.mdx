...
5. MIME-type + magic-byte signature validation — `[AcceptedFileTypes]` if set (MIME types only; extensions are Angular-only), otherwise the image-only default set (`image/jpeg`, `image/png`, `image/gif`, `image/webp`, `image/bmp`). The server reads the first 16 bytes of the stream and rejects requests whose content does not match the declared `Content-Type` — spoofing the header does not bypass validation.
