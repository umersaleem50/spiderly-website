...
The Angular HTTP loading interceptor shows a global spinner for every request by default. Mark a controller method with `[SkipSpinner]` when the request is too fast to need one, runs in the background, or has its own inline loading UI.
