...
Mark a controller method with `[SkipSpinner]` only when the inference can't see your intent: a `[HttpGet]` that returns a full DTO but is **polled / refreshed on a timer**, a background submit, or an endpoint with its own inline loading UI. The example below is a background `void` GET (which resolves to `any`, not a scalar, so it isn't auto-skipped):
